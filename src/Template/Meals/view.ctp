...
-->        <tr>
    ...
    <div class="related">
        <h4><?= __('Related Clients') ?></h4>
         <!--<?php/* $this->request->getSession()->write('Meal.id', $meal->id);
    ...
    echo $this->Html->link(__('New Comment'), ['controller' => 'Clients', 'action' => 'add']);*/?>-->
        ...
        <table cellpadding="0" cellspacing="0">
            <tr>
                <th scope="col"><?= __('Détail') ?></th>
                <th scope="col"><?= __('Nom du plat') ?></th>
            ...
            <tr>
                <td><?= h($clients->detail) ?></td>
                <td><?= h($meal->nom) ?></td>
                <td><?= h($clients->note) ?></td>
                <td><?= h($clients->commentaire) ?></td>
    ...
    $this->request->getSession()->write('Meal.slug', $meal->slug);
    echo $this->Html->link(__('New Comment'), ['controller' => 'Clients', 'action' => 'add']);*/?>-->
        <?php if (!empty($meal->files)): ?>
        <table cellpadding="0" cellspacing="0">
           
            <?php foreach ($meal->files as $files): ?>
            <tr>
                <td><?php 
